...
def test_manual_transaction_transfer_and_reconcile(client, org):
    ...
    cash = client.post(
        ...
        headers=h,
        json={"name": "Cash Drawer", "type": "cash", "openingBalance": 0, "openingBalanceDate": "2026-04-01"},
    ).json()
    ...
    tr = client.post(
        ...
        headers=h,
        json={"fromAccountId": org["bank"]["id"], "toAccountId": cash["id"], "date": "2026-09-01", "amount": 5000},
    )
    ...
    fee = client.post(
        ...
        json={
            ...
            "counterAccountId": org["accounts"]["6100"]["id"],
        },
    )
    assert fee.status_code == 201, fee.text
    txs = client.get("/api/banking/transactions", headers=h, params={"bank_account_id": org["bank"]["id"], "reconciled": False}).json()[
        "items"
    ]
    ...
    reconciled = client.get(
        "/api/banking/transactions", headers=h, params={"bank_account_id": org["bank"]["id"], "reconciled": True}
    ).json()["items"]
    ...
    assert (
        client.post(
            "/api/banking/transfers",
            headers=h,
            json={"fromAccountId": cash["id"], "toAccountId": cash["id"], "date": "2026-09-01", "amount": 1},
        ).status_code
        == 400
    )
...
def test_chart_of_accounts_management(client, org):
    ...
    res = client.post(
        ...
        headers=h,
        json={"code": "6555", "name": "Software Subscriptions", "type": "expense", "subtype": "operating"},
    )
...
def test_manual_journal_must_balance_and_can_be_reversed(client, org):
    ...
    unbalanced = client.post(
        ...
        headers=h,
        json={"date": "2026-09-01", "lines": [{"accountId": a, "debit": 100}, {"accountId": b, "credit": 90}]},
    )
    assert unbalanced.status_code == 400
    both_sides = client.post(
        "/api/accounting/journals",
        headers=h,
        json={"date": "2026-09-01", "lines": [{"accountId": a, "debit": 100, "credit": 100}, {"accountId": b, "credit": 90}]},
    )
    ...
    ok = client.post(
        ...
        json={
            "date": "2026-09-01",
            "reference": "Adj-1",
            "lines": [{"accountId": a, "debit": 100, "description": "Supplies"}, {"accountId": b, "credit": 100}],
        },
    )
