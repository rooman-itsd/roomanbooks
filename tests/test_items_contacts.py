...
def test_item_validation(client, org):
    ...
    svc = client.post(
        "/api/items", headers=h, json={"name": "Svc", "sku": "SVC-X", "type": "service", "trackInventory": True, "openingStock": 5}
    ).json()
...
def test_item_update_and_delete(client, org):
    ...
    created = client.post(
        "/api/items",
        headers=h,
        json={"name": "Temp", "sku": "TMP-1", "sellingPrice": 10, "trackInventory": True, "openingStock": 4, "costPrice": 2},
    ).json()
...
def test_inventory_adjustment_changes_stock_and_posts(client, org):
    ...
    res = client.post(
        ...
        headers=h,
        json={"itemId": org["item"]["id"], "date": "2026-09-01", "quantityDelta": -2, "reason": "Damaged"},
    )
    ...
    assert (
        client.post(
            "/api/inventory-adjustments",
            headers=h,
            json={"itemId": org["item"]["id"], "date": "2026-09-01", "quantityDelta": -999, "reason": "x"},
        ).status_code
        == 400
    )
    assert (
        client.post(
            "/api/inventory-adjustments",
            headers=h,
            json={"itemId": org["service"]["id"], "date": "2026-09-01", "quantityDelta": 1, "reason": "x"},
        ).status_code
        == 400
    )
    # restore
    client.post(
        "/api/inventory-adjustments",
        headers=h,
        json={"itemId": org["item"]["id"], "date": "2026-09-01", "quantityDelta": 2, "reason": "Recount"},
    )
    assert trial_balance_ok(client, h)
...
def test_contacts_crud_and_filters(client, org):
    ...
    created = client.post(
        "/api/contacts",
        headers=h,
        json={"type": "customer", "displayName": "Beta Corp", "gstin": "27AAAAA0000A1Z5", "gstTreatment": "registered_business"},
    ).json()
