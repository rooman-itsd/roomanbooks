...
def to_out(db: Session, acct: BankAccount, unreconciled: Optional[int] = None) -> BankAccountOut:
    return BankAccountOut(
        ...
        account_number_masked=mask_number(acct.account_number),
        ifsc=acct.ifsc,
        currency=acct.currency,
        opening_balance=acct.opening_balance,
        opening_balance_date=acct.opening_balance_date,
        current_balance=bank.current_balance(db, acct),
        unreconciled_count=unreconciled or 0,
        is_active=acct.is_active,
        is_primary=acct.is_primary,
        ledger_account_id=acct.ledger_account_id,
...
def update_account(
    account_id: str, payload: BankAccountUpdate, user: User = Depends(require_financial_write), db: Session = Depends(get_db)
):
...
def list_transactions(
    ...
    stmt = (
        ...
        .where(BankTransaction.organization_id == user.organization_id)
...
def create_transaction(
    account_id: str, payload: BankTransactionCreate, user: User = Depends(require_financial_write), db: Session = Depends(get_db)
):
    org_id = user.organization_id
    ...
    if payload.type == "deposit":
        lines = [
            (acct.ledger_account_id, amount, Decimal("0"), payload.description, None),
            (counter.id, Decimal("0"), amount, payload.description, None),
        ]
    else:
        lines = [
            (counter.id, amount, Decimal("0"), payload.description, None),
            (acct.ledger_account_id, Decimal("0"), amount, payload.description, None),
        ]
    if payload.type == "withdrawal":
        bank.check_cash_overdraft(db, acct, amount)
    tx = bank.record_movement(
        db, acct, payload.date, payload.type, amount, payload.description, "manual", None, user.id, payload.reference, counter.id
    )
...
def transfer(payload: TransferCreate, user: User = Depends(require_financial_write), db: Session = Depends(get_db)):
    ...
    bank.record_movement(
        db, src, payload.date, "withdrawal", amount, desc, "transfer", entry.id, user.id, payload.reference, dst.ledger_account_id, entry.id
    )
    bank.record_movement(
        db, dst, payload.date, "deposit", amount, desc, "transfer", entry.id, user.id, payload.reference, src.ledger_account_id, entry.id
    )
...
def delete_transaction(transaction_id: str, user: User = Depends(require_financial_write), db: Session = Depends(get_db)):
    ...
    if tx.source_type not in ("manual", "transfer"):
        raise HTTPException(
            ...
            f"This transaction was created by a {tx.source_type.replace('_', ' ')}. Delete that record instead.",
        )
    if tx.is_reconciled:
...
def reconcile(payload: ReconcileRequest, user: User = Depends(require_financial_write), db: Session = Depends(get_db)):
    rows = (
        db.execute(
            select(BankTransaction).where(
                BankTransaction.organization_id == user.organization_id, BankTransaction.id.in_(payload.transaction_ids)
            )
        )
        .scalars()
        .all()
    ...
    audit.record(
        db, user, "update", "bank_transaction", None, f"{'Reconciled' if payload.reconciled else 'Unreconciled'} {len(rows)} transactions"
    )
    db.commit()
