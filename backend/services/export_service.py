...
def generate_user_dashboard_pdf(
    target_user: User,
    ...
    """Generate a comprehensive Executive User Dashboard & Activity Report PDF."""
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(
        buffer,
        pagesize=letter,
    ...
    org_name = (org.name if org else None) or "Rooman Books"
    org_addr = (org.address if org else None) or ""
    org_city_state = ", ".join(filter(None, [org.city if org else None, org.state if org else None, org.postal_code if org else None]))
    org_gstin = (org.gstin if org else None) or ""
    org_email = (org.email if org else None) or ""

    header_left_text = f"<b>{org_name}</b>"
    sub_lines = []
    if org_addr:
        sub_lines.append(org_addr)
    if org_city_state:
        sub_lines.append(org_city_state)
    if org_gstin:
        sub_lines.append(f"GSTIN: {org_gstin}")
    if org_email:
        sub_lines.append(f"Email: {org_email}")

    now_str = datetime.now().strftime("%d %b %Y, %I:%M %p")
    header_table_data = [
        [
            Paragraph(header_left_text, title_style),
            Paragraph("<b>USER DASHBOARD REPORT</b>", subtitle_style),
        ],
        [
            Paragraph("<br/>".join(sub_lines) if sub_lines else "Administrative Services", value_style),
            Paragraph(
                f"<b>Generated:</b> {now_str}<br/>"
                f"<b>Target User:</b> {target_user.name}<br/>"
                f"<b>Role:</b> {target_user.role.capitalize()}<br/>"
                f"<b>Status:</b> {'Active' if target_user.is_active else 'Deactivated'}",
                value_style,
            ),
        ],
    ]
    ...
    if employee:
        story.append(Paragraph("Employee & Compensation Profile", sec_heading))
        story.append(Spacer(1, 6))
        basic = float(employee.basic_salary or 0)
        hra = float(employee.hra or 0)
        allowances = float(employee.other_allowances or 0)
        gross = basic + hra + allowances
        pt = float(employee.professional_tax or 0)
        pf = float(employee.pf_employee or 0)
    ...
    # Recent Payslips
    if payslips:
        story.append(Paragraph("Recent Payslips", sec_heading))
        story.append(Spacer(1, 6))
        ps_data = [
            [
                Paragraph("Period", header_th),
                Paragraph("Gross Pay", ParagraphStyle("THRight", parent=header_th, alignment=2)),
                Paragraph("Deductions", ParagraphStyle("THRight", parent=header_th, alignment=2)),
                Paragraph("Net Pay", ParagraphStyle("THRight", parent=header_th, alignment=2)),
                Paragraph("Status", header_th),
                Paragraph("Pay Date", header_th),
            ]
        ]
        for p in payslips[:10]:
    ...
    # Recent Time Entries
    if time_entries:
        story.append(Paragraph("Recent Time Entries", sec_heading))
        story.append(Spacer(1, 6))
        te_data = [
            [
                Paragraph("Date", header_th),
                Paragraph("Project", header_th),
                Paragraph("Hours", ParagraphStyle("THRight", parent=header_th, alignment=2)),
                Paragraph("Billable", header_th),
                Paragraph("Notes", header_th),
            ]
        ]
    ...
    story.append(Paragraph("System Activity & Audit Log", sec_heading))
    story.append(Spacer(1, 6))
    if audit_logs:
        al_data = [
            [
                Paragraph("Date & Time", header_th),
                Paragraph("Action", header_th),
                Paragraph("Entity", header_th),
                Paragraph("Summary", header_th),
            ]
        ]
